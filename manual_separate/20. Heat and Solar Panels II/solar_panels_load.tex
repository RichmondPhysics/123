...
\textbf{Investigate}
\begin{enumerate}
	\item Connect voltage and current sensors. Use \textbf{Help (?)} if necessary.
	\item Insert banana plug leads into sensors if necessary. Use red for (+) and black for (-).
	\item Build a page with 2 digits displays. Choose Voltage for the first digits display and Current for the second digits display. Change the units for Current from amps (A) to milliamps (mA). Use Help (?) if necessary.
	\item In the Sampling Options menu, change Sampling Mode to Manual, change Sampling Rate to 2, and change Sampling Rate Units to seconds. Use Help (?) if necessary.
	\item Set the lamp on its side 30 cm from the panel. Use the panel holder from a previous activity to position the panel at $90^\circ$. Turn on the lamp.
	\item Start collecting data.
	\item Build the zero load circuit with the voltage sensor shown in Figure \ref{zero_load}.
\begin{center}
\begin{figure}[h]
	\begin{center}
	\includegraphics[width=0.45\linewidth]{solar_panels/zero_load.png} 
	\caption{Zero load circuit for voltage or current sensor.}
	\label{zero_load}
	\end{center}
\end{figure}
\end{center}
	\item Find the highest reading over 30 seconds. Use this method to collect data throughout this activity.
	\item Record voltage in Table 1 for 0 loads.
	\item Repeat Steps 7--9 replacing the voltage sensor with the current sensor. Record current for 0 loads in Table 1.
	\item Add alligator clip adapters to the voltage sensor leads. Build the 1-load resistor circuit shown in Figure \ref{one_load}a.
\begin{center}
\begin{figure}[h]
	\begin{center}
\includegraphics[width=0.35\linewidth]{solar_panels/one_load_resistor.png} 
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.35\linewidth]{solar_panels/one_load_led.png} 
\caption{One-load circuit with LED for (a) voltage sensor and (b) current sensor.}
\label{one_load_led}
\end{center}
\end{figure}
\end{center}
...
\begin{figure}[h]
	\begin{center}
	\includegraphics[width=0.32\linewidth]{solar_panels/two_load_voltage.png} 
...
\begin{figure}[h]
	\begin{center}
	\includegraphics[width=0.32\linewidth]{solar_panels/two_load_current.png} 
...
\end{enumerate}
	...
	\begin{figure}[h]
	\begin{center}
	\includegraphics[width=0.30\linewidth]{solar_panels/three_load_voltage.png} 
...
\end{figure}
	...
	\begin{figure}[h]
	\begin{center}
	\includegraphics[width=0.55\linewidth]{solar_panels/three_load_current.png} 
