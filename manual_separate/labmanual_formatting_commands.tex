...
\newcommand{\Lab}[2]{%
  \setlabnumber{#1}%
  \section{#2}% <-- no backslash before #2
}
...
} %for embedded icons that scale to size of text
...
\usepackage[shortlabels]{enumitem} %added 3/2016 by MT.  Tested 100% with other labs, looks like it does no harm.
...
%The following code can be used to make each lab start on its own page--should only be used for electronic copies.
\newcommand{\ForceSectionNewPage}{
  \renewcommand{\section}{\clearpage\NoExtraPageSection}
  \renewcommand{\part}{\clearpage\NoExtraPagePart}
}
